...
class Xref_Job(Core_Job):
    ...
    def xr_build_style_xref_clean_vans(self):
        ...
        self.whouse_class_df = self.redshift_table_to_dataframe(
            redshift_table=response["xref_params"]["xref_input_class"]
        )
        self.whouse_sku_df = self.redshift_table_to_dataframe(
            redshift_table=response["xref_params"]["xref_input_sku"]
        )
        self.whouse_us_item_master_df = self.redshift_table_to_dataframe(
            redshift_table=response["xref_params"]["xref_input_vans_item_master"]
        )
        self.whouse_prod_xref_df = self.redshift_table_to_dataframe(
            redshift_table=response["xref_params"]["xref_input_productxref"]
        )
        self.whouse_vans_mte_style_id_df = self.redshift_table_to_dataframe(
            redshift_table=response["xref_params"]["xref_input_vans_mte_style"]
        )
        self.whouse_vans_peanuts_style_df = self.redshift_table_to_dataframe(
            redshift_table=response["xref_params"]["xref_input_peanuts"]
        )
        self.whouse_vans_prod_seg_past_recom_df = self.redshift_table_to_dataframe(
            redshift_table=response["xref_params"]["xref_input_vans_prod_seg"]
        )
        self.whouse_style_df = self.redshift_table_to_dataframe(
