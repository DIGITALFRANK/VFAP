...
class tr_adobe_ecomm(Dataprocessor_Job):
    ...
    def transform(self, df):
        ...
        if file_name.lower().find('tnf_us') != -1:
            Revenue = "Revenue (Do NOT Use)"
        else:
            Revenue = "Revenue"
        ...
        try:
            ...
            df_revenue = df_product_view.withColumnRenamed(Revenue, "REVENUE")
            df_sales_eur = df_revenue.withColumn("TOTAL_SALES_EUR",
                                                      when((col('REGION') ==
                                                            'EMEA'),
                                                           col(
                                                               'REVENUE')).otherwise(
            ...
            df_remove_null = df_sales_usd.where("Date is NOT NULL")
            # format date column
            ...
            df_replace_devicecategory = df_date.withColumn(
                "Mobile Device Type", regexp_replace(col("Mobile Device "
                                                         "Type"), "Other",
                                                     "Desktop"))
