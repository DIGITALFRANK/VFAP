...
class Dataprocessor_merge:
    def __init__(self, file_pattern, date):
        ...
        self.logger = logger
        self.params = utils.get_file_config_params(file_pattern, self.logger)
    ...
    def read_from_s3(self, delimiter, bucket, path):
        ...
        try:
            ...
            logger.info(
                 "Error Ocuurred While reading file : {} from s3 due to : {}".format(s3_obj, error)
                         )
        self.df = df
        return df
    ...
    def update_stage_status(
        ...
        try:
            stage_update_status = DynamoUtils.update_dndb_items(
                ...
                partition_key_value=file_pattern,
    ...
    @staticmethod
    def process(file_pattern, date, job_process_dttm):
        ...
        try:
            #Condition to check whether substring is present in filename
            if file_pattern.lower().find('ecommerge') != -1:
            # import the class dynamically for given input
                klass = utils.get_dynamic_class(transformation_dict["1"], logger)
                tr_obj = klass(file_pattern, date)
                df1 = tr_obj.read_from_s3(delimiter,
                    bucket=env_params["transformed_bucket"],
                    path=env_params["adobe_source_dir"] + "/" + date + "/*")
                df2 = tr_obj.read_from_s3(delimiter,
                    bucket=env_params["transformed_bucket"],
                    path=env_params["google_source_dir"] + "/" + date + "/*")
                df3 = tr_obj.read_from_s3(delimiter,
                    bucket=env_params["transformed_bucket"],
                    path=env_params["coremetrics_source_dir"] + "/" + date + "/*")
                df_merged_initial = df1.union(df2)
                ...
                print("Reading files to merge")
                klass = utils.get_dynamic_class(transformation_dict["1"],
                                                logger)
                tr_obj = klass(file_pattern, date)
                df = tr_obj.read_from_s3(delimiter,
                    bucket=env_params["transformed_bucket"],
                    path=params["rf_source_dir"] + '/' + date + '/*')
                df.show()
                transformed_df = tr_obj.transform(df)
                transformed_df.show()
